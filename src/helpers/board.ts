...
export class Board {
  ...
  field: number[][];
  player: number;
  constructor(game : Game, field: number[][], player : number){
    this.game = game;
    this.field = field;
    this.player = player;
  }
 ...
 * @return {boolean}
 */
  isFinished(depth : number, score : number) {
  if (
    depth == 0 ||
 ...
 * @return {number}
 */
  scorePosition(row : number, column : number, delta_y : number, delta_x : number) {
  var human_points = 0;
